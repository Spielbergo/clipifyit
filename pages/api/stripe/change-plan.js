...
export default async function handler(req, res) {
  ...
  try {
    ...
    const planKey = typeof targetPlan === 'string' ? targetPlan.toLowerCase() : '';
    if (!['pro','proplus'].includes(planKey)) return res.status(400).json({ error: 'Invalid targetPlan' });
    ...
    const subsList = await stripe.subscriptions.list({ customer: profile.stripe_customer_id, status: 'all', limit: 5 });
    ...
    const termKey = interval === 'year' ? 'yearly' : 'monthly';
